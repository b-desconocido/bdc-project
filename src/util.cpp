...
#else
    const char* pszModule = "blackdragoncoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BlackDragonCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "BlackDragonCoin";
#else
    // Unix
    return pathRet / ".blackdragoncoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "blackdragoncoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "blackdragoncoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Black Dragon Coin will not work properly.");
