...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
		...
		SetMinVersion(FEATURE_BASE, pwalletdbEncryption, true);
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
			...
			if ((wtx.IsCoinBase() || wtx.IsRollBase()) && wtx.IsSpent(0))
                continue;
...
void CWalletTx::RelayWalletTransaction()
{
    BOOST_FOREACH(const CMerkleTx& tx, vtxPrev)
    {
		...
		if ((!tx.IsCoinBase() && !tx.IsRollBase()) && !tx.vin.empty())
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
			...
			if ((pcoin->IsCoinBase() || pcoin->IsRollBase()) && pcoin->GetBlocksToMaturity() > 0)
                continue;
