...
static const int COINBASE_MATURITY = 980;
...
public:
    ...
    */
	...
	// Extract unique public keys from tx
	void GetUniquePubKeys(boost::unordered::unordered_set<CScript> &out) const;
...
public:
    ...
    CTxInUndo() : txout(), fCoinBase(false), fRollBase(false), nHeight(0), nVersion(0) {}
	CTxInUndo(const CTxOut &txoutIn, bool fCoinBaseIn = false, bool fRollBaseIn = false, unsigned int nHeightIn = 0, int nVersionIn = 0) : txout(txoutIn), fCoinBase(fCoinBaseIn), fRollBase(fRollBaseIn), nHeight(nHeightIn), nVersion(nVersionIn) { }

    unsigned int GetSerializeSize(int nType, int nVersion) const {
		return ::GetSerializeSize(VARINT(nHeight * 4 + (fCoinBase ? 1 : 0) + (fRollBase ? 2 : 0)), nType, nVersion) +
    ...
    void Serialize(Stream &s, int nType, int nVersion) const {
        ::Serialize(s, VARINT(nHeight * 4 + (fCoinBase ? 1 : 0) + (fRollBase ? 2 : 0)), nType, nVersion);
        if (nHeight > 0)
...
public:
	...
	CCoins(const CTransaction &tx, int nHeightIn) : fCoinBase(tx.IsCoinBase()), fRollBase(tx.IsRollBase()), vout(tx.vout), nHeight(nHeightIn), nVersion(tx.nVersion) { }

    // empty constructor
	CCoins() : fCoinBase(false), fRollBase(false), vout(0), nHeight(0), nVersion(0) { }
    ...
    void swap(CCoins &to) {
		...
		std::swap(to.fRollBase, fRollBase);
        to.vout.swap(vout);
    ...
    unsigned int GetSerializeSize(int nType, int nVersion) const {
		...
		unsigned int nCode = 16*(nMaskCode - (fFirst || fSecond ? 0 : 1)) + (fCoinBase ? 1 : 0) + (fRollBase ? 2 : 0) + (fFirst ? 4 : 0) + (fSecond ? 8 : 0);
        // version
...
public:
    ...
    {
        uint256 thash;
		scrypt_jane_N_1_1_256(GetNFactor(nTime), BEGIN(nVersion), BEGIN(thash));
        return thash;
    }
...
class CDice
{
	/* Если выпадает 13 - удваиваем награду */
	const static int nCriticalValue = 13;
	int64 _seed;
	int64 _baseReward;
	int64 _prevRollReward, _prevBaseReward;
	bool _isCritical;
	int Roll(int range);
	int64 RollDice(int height);
public:
	CDice(int currentBlockHeight, int prevBlockHeight, int64 seed);

	static bool HasRandomReward(int prevBlockHeight)
	{
		return true;
	}

	int64 GetPrevBaseSubisidy(int64 nFees) const
	{
		return _prevBaseReward * COIN + nFees;
	}

	int64 GetPrevRollSubsidy() const
	{
		return _prevRollReward;
	}

	int64 GetCurrentBaseSubsidy(uint64 nFees) const
	{
		return _baseReward * COIN + nFees;
	}
	...
	static int64 GetBaseReward(int height)
	{
		if(height == 1)
		{
			return 215346;
	...
	static int64 GetMinRollReward()
	{
		return DUST_HARD_LIMIT;
	}
};
