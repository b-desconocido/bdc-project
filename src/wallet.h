...
enum WalletFeature
{
	FEATURE_BASE = CLIENT_VERSION, // the earliest version new wallets supports (only useful for getinfo's clientversion output)
...
public:
    ...
    int64 GetCredit(bool fUseCache=true) const
    {
		...
		if ((IsCoinBase() || IsRollBase()) && GetBlocksToMaturity() > 0)
            return 0;
    ...
    {
		if ((IsCoinBase() || IsRollBase()) && GetBlocksToMaturity() > 0 && IsInMainChain())
        {
